...
def _hist_panel_grid(
    *,
    ...
    title_clean = _pretty(title)
    wrapped_title = "\n".join(textwrap.wrap(title_clean, width=80))
    
    fig.suptitle(wrapped_title, y=0.98, fontsize=11)
    ...
    *,
    ...
    def _plot_group(group_key: str, v: Dict[str, Any]) -> None:
        ...
        for stat_key in STAT_KEYS:
            ...
            out_dir = Path(base_dir) / _safe_stem(prefix) / _safe_stem(horizon_label) / _safe_stem(equation_name) / _safe_stem(group_key)
            
            # FORCE FULL DIRECTORY CREATION HERE
...
def _df_to_booktabs_latex_with_title_and_caption_below(
    ...
    *,
    ...
    lines.append(r"\tiny")
    lines.append(r"\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}")
    lines.append(rf"\caption{{{title}}}\par") #####
