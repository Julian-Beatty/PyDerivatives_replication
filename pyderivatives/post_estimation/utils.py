...
def call_fit_error_timeseries(
    ...
    *,
    ...
    # Trim observations above the original 95th percentile before summary stats/plots
    clip_at_p95: bool = True,
    ...
    def _trim_above_p95(s: pd.Series) -> pd.Series:
        s = pd.to_numeric(s, errors="coerce").dropna()
        if clip_at_p95 and not s.empty:
            p95 = s.quantile(0.95)
            s = s[s <= p95]
        return s
    ...
    if plot:
        ...
        if save_plots:
            if plot_dir is None:
                raise ValueError("plot_dir must be provided if save_plots=True")
        ...
        for m in metrics:
            ...
            series = pd.to_numeric(ts_df[m], errors="coerce")
            ...
            if clip_at_p95:
                ...
                series_plot = series.where(series <= p95)
            else:
                series_plot = series

            plt.figure(figsize=(10, 3.2))
            plt.plot(ts_df[date_col], series_plot)
            ...
            if save_plots:
                fname = f"{m}_timeseries.{plot_format}"
    ...
    if any(str(m).lower() in percent_set for m in tab.index):
        ...
        lines.append(
            ...
            r"Summary statistics are computed after excluding observations above the 95th percentile. "
            r"\end{minipage}"
        )
