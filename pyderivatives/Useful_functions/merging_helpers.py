...
def slice_dict_by_date(data_dict, start_date, end_date, date_format="%Y-%m-%d"):
    """
    Slice a dictionary with date-string keys between start_date and end_date (inclusive).

    Parameters
    ----------
    data_dict : dict
        Dictionary with date-string keys.
    start_date : str
        Start date (inclusive), e.g. "2008-01-01"
    end_date : str
        End date (inclusive), e.g. "2019-12-31"
    date_format : str
        Format of the date strings (default "%Y-%m-%d")
    ...
    return {
        ...
        if start <= datetime.strptime(date, date_format) <= end
    }
